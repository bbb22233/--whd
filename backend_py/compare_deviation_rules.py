...
PAYLOAD_KEYS = ["finalWeather", "currentRuleRows", "ruleLibraryRows"]
OPTIONAL_KEYS = {"generatedAt", "metricBucketMode", "confidenceLimited"}
...
def main(argv: list[str] | None = None) -> None:
    ...
    node_path = REPORTS_DIR / f"{stem}_deviation_rules{node_suffix}.json"
    python_path = REPORTS_DIR / f"{stem}_deviation_rules{python_suffix}.json"
    ...
    if failures:
        print(json.dumps({"status": "failed", "failureCount": len(failures), "failures": failures[:100]}, ensure_ascii=False, indent=2))
        raise SystemExit(1)
        ...
        json.dumps(
            {
                ...
                "currentRuleRowCount": len(python_payload["currentRuleRows"]),
                "ruleLibraryRowCount": len(python_payload["ruleLibraryRows"]),
                "finalGate": python_payload["finalWeather"].get("gate"),
            },
            ensure_ascii=False,
            indent=2,
        )
