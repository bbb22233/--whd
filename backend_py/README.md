...
Python 后端接管 API、官方报告生成、数据下载/清洗与一键 parity 回归。
Node 目前只保留静态前端服务入口 `server.mjs`。
...
- 现在 `summary`、`full` scanner 入口和常用单品种研究入口已切到 Python official pipeline,正式 reports 与 summaries 由 Python 生成。
- parity 回归使用冻结 fixture/golden,不再需要现场运行 Node research code。
- Python 负责读取 reports 并对前端提供 API。
- Node 仅保留静态前端服务入口 `serve`;如果后续改成 FastAPI 静态托管,可一并移除。
